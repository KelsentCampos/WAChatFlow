...
using WAChatFlow.Shared.Models.Dtos;
...
namespace WAChatFlow.Client.Pages
{
    public class UsersBase : ComponentBase
    {
        ...
        protected List<UsersDto.UserStatusRow> users = new();

        protected override async Task OnInitializedAsync()
        {
            try
            {
                var data = await Http.GetFromJsonAsync<IEnumerable<UsersDto.UserStatusRow>>("api/users/catalog"); users = data?.ToList() ?? new List<UsersDto.UserStatusRow>();
            }
            catch (Exception ex)
            {
        ...
        protected RenderFragment GetActionButton(UsersDto.UserStatusRow user)
        {
            return builder =>
            {
        ...
        {
            return PhoneUtils.FormatPhoneForDisplay(phoneNumber);
        }

        protected async Task EnviarConsentimiento(UsersDto.UserStatusRow user)
        {
            try
            {
                var recipientWaId = PhoneUtils.NormalizeToWhatsAppId(user.PhoneNumber);
        ...
        protected async Task Reenviar(UsersDto.UserStatusRow user)
        {
            Console.WriteLine($"🔄 Reenviando a {user.FullName}");
            await EnviarConsentimiento(user);
        }

        protected async Task Reintentar(UsersDto.UserStatusRow user)
        {
            Console.WriteLine($"⏳ Reintentando con {user.FullName}");
            await EnviarConsentimiento(user);
        }

        protected async Task DisableEditAsync(UsersDto.UserStatusRow u)
        {
            string newName;
        ...
        protected bool IsEditing(UsersDto.UserStatusRow u)
        {
            if (_editing.Contains(u.UserId))
            {
                return true;
        ...
        protected void EnableEdit(UsersDto.UserStatusRow u)
        {
            if (!_editing.Contains(u.UserId))
            {
        ...
        protected string GetBuffer(UsersDto.UserStatusRow u)
        {
        ...
        protected void OnNameChanged(UsersDto.UserStatusRow u, string value)
        {
            if (_editBuffer.ContainsKey(u.UserId))
            {
        ...
        protected async Task OnNameKeyDown(KeyboardEventArgs e, UsersDto.UserStatusRow u)
        {
            if (e.Key == "Enter")
            {
        ...
        protected void CancelEdit(UsersDto.UserStatusRow u)
        {
            if (_editing.Contains(u.UserId))
            {
                _editing.Remove(u.UserId);
